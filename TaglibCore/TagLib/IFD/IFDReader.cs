...
#region Private Methods
		...
		private static readonly Dictionary<File, List<long>> ifd_offsets = new();
		private static readonly Dictionary<File, int> ifd_loopdetect_refs = new();
		...
		private IFDEntry ParseMakernote (ushort tag, ushort type, uint count, long base_offset, uint offset)
		{
			...
			IFDStructure ifd_structure = new();
			...
			if (header.StartsWith (PANASONIC_HEADER)) {
				IFDReader reader =
					new(file, is_bigendian, ifd_structure, base_offset, offset + 12, max_offset);
			...
			if (header.StartsWith (PENTAX_HEADER)) {
				IFDReader reader =
					new(file, is_bigendian, ifd_structure, base_offset, offset + 6, max_offset);
			...
			if (header.StartsWith (OLYMPUS1_HEADER)) {
				IFDReader reader =
					new(file, is_bigendian, ifd_structure, base_offset, offset + 8, max_offset);
			...
			if (header.StartsWith (OLYMPUS2_HEADER)) {
				IFDReader reader =
					new(file, is_bigendian, ifd_structure, makernote_offset, 12, count);
			...
			if (header.StartsWith (LEICA_HEADER)) {
				IFDReader reader = new(file, is_bigendian, ifd_structure, makernote_offset, 8, count);
			...
			try {
				IFDReader reader =
					new(file, is_bigendian, ifd_structure, base_offset, offset, max_offset);
