...
namespace TagLib.Png
{
    ...
    /// <summary>
    ///    This class extends <see cref="ImageBlockFile" /> to provide tagging
    ///    for PNG image files.
    /// </summary>
    /// <remarks>
...
#region private methods
		...
		///    is not included.
		/// </returns>
		private static string ReadTerminatedString (ByteVector data, int start_index, out int terminator_index)
		{
		...
		///    is not included.
		/// </returns>
		private static string ReadKeyword (ByteVector data, int start_index, out int terminator_index)
		{
		...
		private void ReadiTXtChunk (int data_length)
		{
            ...
            string keyword = ReadKeyword(data, 0, out int terminator_index);
		...
		private void ReadtEXtChunk (int data_length)
		{
            ...
            string keyword = ReadKeyword(data, 0, out int keyword_terminator);
		...
		{
			ByteVector metadata_chunks = new();
		...
		private ByteVector RenderKeywordChunks ()
		{
            ...
            if (GetTag(TagTypes.Png, true) is not PngTag png_tag)
                return null;

            ByteVector chunks = new();
