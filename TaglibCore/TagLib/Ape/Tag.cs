...
#region Private Static Fields
		...
		/// </summary>
		private static readonly string [] picture_item_names = new string [] {
			"Cover Art (other)",
			"Cover Art (icon)",
...
#region Private Fields
		...
		public void AddValue (string key, string [] value)
		{
			...
			Item item = new(key, values.ToArray ());
		...
		private string [] GetItemAsStrings (string key)
		{
			Item item = GetItem (key);
			return item != null ?
				item.ToStringArray () : Array.Empty<string>();
		}
		...
		private uint GetItemAsUInt32 (string key, int index)
		{
            ...
            if (uint.TryParse(values[index], out uint result))
                return result;
