...
#region Parsing speedup
		...
		static readonly NameTable NameTable = new();
		...
		/// </summary>
		public static Dictionary<string, string> NamespacePrefixes = new();
...
#region Constructors
		...
		public XmpTag (string data, File file)
		{
			...
			node ??= doc.SelectSingleNode("/x:xapmeta/rdf:RDF", nsmgr);
			if (node == null)
...
#region Private Methods
		...
		//		nodeElementList
		//		end-element()
		private XmpNode ParseRDF (XmlNode rdf_node, File file)
		{
			XmpNode top = new(String.Empty, String.Empty);
		...
		private void ParseResourcePropertyElement (XmpNode parent, XmlNode node)
		{
			...
			XmpNode new_node = new(node.NamespaceURI, node.LocalName);
			foreach (XmlAttribute attr in node.Attributes) {
				if (attr.Is (XML_NS, LANG_URI)) {
		...
		//		text()
		//		end-element()
		private static void ParseLiteralPropertyElement (XmpNode parent, XmlNode node)
		{
			if (!node.IsPropertyElement ())
				throw new CorruptFileException ("Invalid property");
		...
		//		end-element()
		private static void ParseEmptyPropertyElement (XmpNode parent, XmlNode node)
		{
			if (!node.IsPropertyElement ())
				throw new CorruptFileException ("Invalid property");
			if (node.HasChildNodes)
		...
		private static XmpNode CreateTextPropertyWithQualifiers (XmlNode node, string value)
		{
			XmpNode t = new(node.NamespaceURI, node.LocalName, value);
			foreach (XmlAttribute attr in node.Attributes) {
				if (attr.In (XMLNS_NS))
					continue;
...
#region Public Methods
		...
		public double? GetRationalNode (string ns, string name)
		{
            ...
            if (Double.TryParse(values[0], out double nom) && Double.TryParse(values[1], out double den))
            {
                if (den != 0.0)
		...
		private static string DecimalToFraction (double value, long max_denominator) {
			var m = new long [2, 2];
			m[0, 0] = m[1, 1] = 1;
			m[1, 0] = m[0, 1] = 0;
		...
		public uint? GetUIntNode (string ns, string name)
		{
            ...
            if (UInt32.TryParse(text, out uint result))
                return result;
		...
		private class NodeIndexVisitor : IXmpNodeVisitor
		{
			private readonly XmpTag tag;
...
#region Metadata fields
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> containing the rating of the
        ///    current instace.
        /// </value>
        public override uint? Rating {
			get { return GetUIntNode (XAP_NS, "Rating"); }
			set { SetTextNode (XAP_NS, "Rating", value?.ToString ());
			}
		}
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the time the image was taken.
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="ImageOrientation" /> containing the orientation of the
        ///    image
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the latitude ranging from -90.0
        ///    to +90.0 degrees.
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the longitude ranging from -180.0
        ///    to +180.0 degrees.
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the altitude. A positive value
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the exposure time in seconds.
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the FNumber.
        /// </value>
        /// <remarks>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the ISO speed as defined in ISO 12232.
        /// </value>
        /// <remarks>
        ...
        public override uint? ISOSpeedRatings {
			get {
				...
				if (values != null && values.Length > 0) {
                    if (UInt32.TryParse(values[0], out uint result))
                        return result;
                }
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the focal length in millimeters.
        /// </value>
        ...
        /// </summary>
        /// <value>
        ///    A <see cref="Nullable"/> with the focal length in 35mm equivalent in millimeters.
        /// </value>
