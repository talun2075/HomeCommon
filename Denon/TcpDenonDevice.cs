...
namespace Denon
{
    public class TcpDenonDevice : IDenonDevice, IDisposable
    {
        ...
        private string GetResponse()
        {
            ...
            return Encoding.ASCII.GetString(stream.GetBuffer()).Replace("\0", string.Empty);
        }
        #endregion
        #region Init
        private void Init()
        {
            ...
            var results = result.Split('\r', StringSplitOptions.RemoveEmptyEntries);
            foreach (var s in results)
            {
                if (s.StartsWith("SI"))
                {
                    var input = s.Substring(2);
                    Input = StringToInputSourceLookup[input];
                }
                if (s.StartsWith("SV"))
                {
                    var power = s.Substring(2);
                    if(power=="ON") PowerOn = true;
                }
            }
        }
        #endregion Init
        #region Properties
        public Boolean PowerOn { get; set; } = false;
        public InputSource Input { get; set; } = InputSource.Unknown;
        ...
        public InputSource GetSelectedInputSourceStatus()
        {
            SendMessage("SI?", out string result);
            var results = result.Split('\r', StringSplitOptions.RemoveEmptyEntries);
            foreach (var s in results)
            {
                if(!s.StartsWith("SI")) continue;

                var input = s.Substring(2);
                var retval = StringToInputSourceLookup[input];
                return retval;
            }
        ...
        private static readonly Dictionary<string, InputSource> StringToInputSourceLookup = new Dictionary<String, InputSource>(){
            { "PHONO",InputSource.Phono},
            { "CD",InputSource.Cd},
            { "TUNER",InputSource.Tuner},
            {"BD", InputSource.BlueRay},
            {"TV", InputSource.Tv},
            { "SAT/CBL",InputSource.SatCbl},
            { "MPLAY",InputSource.MediaPlayer},
            { "GAME",InputSource.Game},
            { "NET",InputSource.Net}
        };
